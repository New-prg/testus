...
from app.db.models import User, Vehicle, VehicleMetricWindow, VehicleRatingWindow, VehicleSensor
...
def _vehicles_with_analytics_query(user: User):
    return (
        select(Vehicle)
...
def _load_vehicles_with_analytics(db: Session, user: User) -> list[Vehicle]:
    return list(db.scalars(_vehicles_with_analytics_query(user)).all())


def _load_vehicle_with_analytics(db: Session, user: User, vehicle_id: str) -> Vehicle | None:
    return db.scalar(_vehicles_with_analytics_query(user).where(Vehicle.id == vehicle_id))
...
def list_vehicles(
    ...
    return [_vehicle_row(vehicle) for vehicle in _load_vehicles_with_analytics(db, current_user)]
...
def vehicle_metrics(
    ...
    current_user: Annotated[User, Depends(deps.get_fleet_access_user)],
    limit: int = 100,
) -> list[VehicleMetricWindow]:
    if not _load_vehicle_with_analytics(db, current_user, vehicle_id):
        raise HTTPException(status_code=404, detail="Машина не найдена")
