...
class RatingExplainer:
    def explain(self, metric_scores: Mapping[str, float | None]) -> tuple[list[str], list[str], list[str]]:
        ...
        positive = [f"high {name}" for name, value in metric_scores.items() if value is not None and value >= 8]
        negative = [f"low {name}" for name, value in metric_scores.items() if value is not None and value <= 4]
